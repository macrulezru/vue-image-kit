...
async function hydrateAndCollectWarnings(html: string, props: Record<string, unknown>): Promise<string[]> {
...
describe('VImage SSR hydration', () => {
  ...
  it('hydrates cleanly with lazy=false (immediate-load) props too', async () => {
    const props = { src: '/img.jpg', alt: 'Mountain landscape', width: 400, height: 300, lazy: false, priority: true }
    const html = await renderServerHtml(props)
